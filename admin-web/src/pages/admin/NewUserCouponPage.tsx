...
export function NewUserCouponPage() {
  ...
  useEffect(() => {
    ...
    getCouponPageData(groupId, 'PROMOTION')
      ...
      .catch((requestError: Error) => {
        if (!active) return
        setError(requestError.message || '加载新人优惠券失败')
        setRows([])
        setGroupName('')
      })
      .finally(() => {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setRows((currentValue) => upsertCouponRow(currentValue, response.data.row))
      setNotice(editorMode === 'edit' ? '优惠券已更新' : '优惠券已创建')
      setPage(1)
      closeEditor(true)
    } catch (requestError) {
      const message = requestError instanceof Error ? requestError.message : ''
    ...
    >
      <div className="admin-resource-page new-user-coupon-page">
        ...
        <section className="admin-resource-panel new-user-coupon-panel">
          <div className="new-user-coupon-section-title">商城优惠券命名约定</div>
          <div className="admin-inline-notice">
            商城当前复用 `PROMOTION` 优惠券，只识别名称或券码包含 `商城` / `MALL` 的券。
            建议直接使用：`商城新人券`、`商城首单券`、`商城复购券`、`商城通用券`，
            或券码 `MALL_NEW`、`MALL_FIRST`、`MALL_REBUY`、`MALL_GENERAL`。
          </div>
        </section>
              ...
              <form onSubmit={(event) => void handleSubmit(event)}>
                <div className="new-user-coupon-modal-header">
                  <div className="new-user-coupon-modal-title">
                    {editorMode === 'create' ? '创建「新人优惠券」' : '编辑「新人优惠券」'}
                  </div>
                  <ul className="new-user-coupon-modal-tips">
                    <li>仅限「新用户」加入星球时使用</li>
                    <li>以后修改星球价格，大于或等于星球价格的优惠券将自动失效</li>
                  </ul>
                </div>
