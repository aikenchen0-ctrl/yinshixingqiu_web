...
func GetByTaskIDOrUpstreamTaskID(userId int, taskId string) (*Task, bool, error) {
	if taskId == "" {
		return nil, false, nil
	}
	...
	case common.UsingPostgreSQL:
		err = query.Where("private_data->>'upstream_task_id' = ?", taskId).First(task).Error
	case common.UsingMySQL:
		err = query.Where("JSON_UNQUOTE(JSON_EXTRACT(private_data, '$.upstream_task_id')) = ?", taskId).First(task).Error
	default:
		err = query.Where("json_extract(private_data, '$.upstream_task_id') = ?", taskId).First(task).Error
	}
	exist, err = RecordExist(err)
	if err != nil {
		return nil, false, err
