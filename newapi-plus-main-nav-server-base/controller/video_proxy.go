...
func VideoProxy(c *gin.Context) {
	...
	default:
		...
		if isTaskProxyContentURL(videoURL, task.TaskID) || strings.TrimSpace(videoURL) == "" {
			if resolvedURL := extractDirectVideoURLFromTask(task); resolvedURL != "" {
...
func extractDirectVideoURLFromTask(task *model.Task) string {
	...
	for _, path := range []string{
		...
		if candidate := strings.TrimSpace(gjson.GetBytes(task.Data, path).String()); isDirectVideoURL(candidate, task.TaskID) {
			return candidate
		}
	}
...
func isDirectVideoURL(videoURL string, taskID string) bool {
	...
	return strings.HasPrefix(videoURL, "http://") ||
		strings.HasPrefix(videoURL, "https://") ||
		strings.HasPrefix(videoURL, "data:")
}
